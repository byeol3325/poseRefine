...
def cropRotImage(angle, ori_path, new_path, train=True):
    if (train == False):
        ori_path += 'testing/'
        new_path += 'testing/'
    else:
    ...
    for file_name in file_list_py:
        ...
        rotate_bound = imutils.rotate_bound(imgL, angle)
        ...
        with open(new_path_one + 'cam.txt', 'w') as file:
            file.writelines(cam)
        ...
        """
        img = cv2.imread(ori_image_path + file_name)
        rotate_bound = imutils.rotate_bound(img, angle)
        r_h, r_w, _ = rotate_bound.shape
        h, w, _ = img.shape
        pi_angle = math.pi/180*angle
        ratio = h * math.sqrt(1 + math.tan(pi_angle)**2) / 2 / (h+w*math.tan(pi_angle))
        new_h = h*ratio
        new_w = w*ratio
        cropped_img = rotate_bound[int(-new_h + r_h/2):int(new_h + r_h/2), int(-new_w + r_w/2):int(new_w + r_w/2), :]
        cv2.imwrite(new_image_path + file_name, cropped_img)
        c_h, c_w, _ = cropped_img.shape
        fixed_img = cropped_img[int(c_h/2-122):int(c_h/2+122),int(c_w/2-402):int(c_w/2+402),:]
        #print(size_image_path + file_name)
        cv2.imwrite(size_image_path + file_name, fixed_img)
        #print(new_image_path + file_name)
        #print(size_image_path + file_name)
        """

    print(angle, ' angle image is fin')

    return None

ori_path = '/data0/data/KITTI/3Dobject/'
new_path = '/data0/data/KITTI/forTEST/forMVS/original_crop/'
angle = 0
cropRotImage(angle, ori_path, new_path, train=True)
